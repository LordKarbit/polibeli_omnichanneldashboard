...
export default function DataQualityPage() {
  ...
  const summary = data?.dataQuality
    ...
    : emptyQualitySummary;
  const metrics = data?.dataQuality?.metrics ?? [];
  const duplicateOrderMetric = metrics.find((metric) => metric.category === 'Repeated order-level rows');
  const zeroItems = data?.dataQuality?.zeroValueItems ?? [];
  const schemaIssues = data?.dataQuality?.schemaMismatches ?? [];
      ...
      <ChartCard title="Duplicate Order Detection" subtitle="Orders appearing multiple times across uploads">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {duplicateOrderMetric && duplicateOrderMetric.count > 0 ? (
                <tr className="border-b border-border/50 transition-colors hover:bg-muted/30">
                  <td className="py-3 pr-4 font-mono text-xs text-foreground">Deduped order-level rows</td>
                  <td className="py-3 pr-4 text-xs text-muted-foreground">All uploaded sources</td>
                  <td className="py-3 pr-4 text-right">
                    <span className="rounded-md bg-amber-500/10 px-2 py-0.5 text-xs font-semibold text-amber-500">
                      {duplicateOrderMetric.count.toLocaleString('id-ID')}
                    </span>
                  </td>
                  <td className="py-3 text-xs text-muted-foreground">{duplicateOrderMetric.description}</td>
                </tr>
              ) : (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-xs text-muted-foreground">
                    No duplicate order-level rows detected in the current dataset.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!zeroItems.length && (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-xs text-muted-foreground">
                    No zero-value or giveaway line items detected.
                  </td>
                </tr>
              )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!schemaIssues.length && (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-xs text-muted-foreground">
                    No schema mismatch detected in uploaded files.
                  </td>
                </tr>
              )}
            </tbody>
