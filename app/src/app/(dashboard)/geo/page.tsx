...
import { MapLibreGeoHeatmap } from '@/components/geo/maplibre-geo-heatmap';
import { customerData, locationData } from '@/data/mock/extended';
...
function getFallbackRetentionAnalytics(): CustomerRetentionAnalytics {
  const channelMap = new Map<string, CustomerRetentionAnalytics['channels'][number]>();
  customerData.forEach((customer) => {
    const channelKey = customer.channel.toLowerCase().replace(/\s+/g, '-');
    ...
    current.totalOrders += customer.orders;
    current.totalGMV += customer.totalGMV;
    if (customer.orders <= 1) current.oneTimeCustomers += 1;
    else if (customer.orders === 2) current.twoTimeCustomers += 1;
    else if (customer.orders <= 5) current.threeToFiveCustomers += 1;
    else current.sixPlusCustomers += 1;
    if (customer.orders > 1) current.repeatCustomers += 1;
  ...
  const summary = channels.reduce<CustomerRetentionAnalytics['summary']>(
    (acc, channel) => {
      acc.uniqueCustomers += channel.uniqueCustomers;
      acc.oneTimeCustomers += channel.oneTimeCustomers;
      acc.twoTimeCustomers += channel.twoTimeCustomers;
      acc.threeToFiveCustomers += channel.threeToFiveCustomers;
      acc.sixPlusCustomers += channel.sixPlusCustomers;
      acc.repeatCustomers += channel.repeatCustomers;
      acc.returningCustomers += channel.returningCustomers;
      acc.totalOrders += channel.totalOrders;
      acc.totalGMV += channel.totalGMV;
      return acc;
    },
    {
      uniqueCustomers: 0,
  ...
  summary.repeatRate = summary.uniqueCustomers ? (summary.repeatCustomers / summary.uniqueCustomers) * 100 : 0;
  summary.avgPurchaseFrequency = summary.uniqueCustomers ? summary.totalOrders / summary.uniqueCustomers : 0;

  return {
    summary,
    channels,
    monthly: channels.map((channel) => ({ ...channel, month: '2026-04', monthLabel: 'Apr 2026' })),
    customers: customerData.map((customer) => ({
      customer: customer.name,
      channel: customer.channel,
      channelKey: customer.channel.toLowerCase().replace(/\s+/g, '-'),
      purchaseCount: customer.orders,
      segment: customer.orders <= 1 ? 'One-time' : customer.orders === 2 ? 'Repeat 2x' : customer.orders <= 5 ? 'Repeat 3-5x' : 'Repeat 6x+',
      activeMonths: 1,
...
export default function GeoSalesPage() {
  ...
  const retentionAnalytics = data?.customerRetentionAnalytics ?? getFallbackRetentionAnalytics();
