...
import { regionalManagers, areaManagers } from '@/data/mock/gt-performance';
...
function buildSalesPerformance(transactions: GeoTransactionSummary[]) {
  const map = new Map<
    string,
    Omit<SalesPerformance, 'customers' | 'cities'> & {
  ...
  transactions.forEach((order) => {
    const name = order.salesName?.trim() || 'Unassigned BD';
    ...
    current.orders += 1;
    current.bookedGMV += order.bookedGMV;
    current.activeGMV += order.activeGMV;
    if (!isCancelled(order.status)) current.activeOrders += 1;
    else current.cancellationValue += order.bookedGMV;
    if (order.customer) current.customerSet.add(order.customer.toLowerCase());
    if (order.city) current.citySet.add(`${order.province ?? ''}|${order.city}`.toLowerCase());
  ...
  return Array.from(map.values())
    .map((sales) => ({
      ...
      customers: sales.customerSet.size,
      cities: sales.citySet.size,
      bookedGMV: sales.bookedGMV,
      activeGMV: sales.activeGMV,
      cancellationValue: sales.cancellationValue,
      lastOrder: sales.lastOrder,
    }))
    .sort((a, b) => b.bookedGMV - a.bookedGMV);
}

function buildCityPerformance(transactions: GeoTransactionSummary[]) {
  const map = new Map<
    string,
    Omit<CityPerformance, 'customers' | 'areaManagers' | 'sales'> & {
      customerSet: Set<string>;
  ...
  transactions.forEach((order) => {
    ...
    current.activeGMV += order.activeGMV;
    if (order.customer) current.customerSet.add(order.customer.toLowerCase());
    if (order.areaManager) current.managerSet.add(order.areaManager);
    if (order.salesName) current.salesSet.add(order.salesName);
    map.set(key, current);
  });
  ...
  return Array.from(map.values())
    .map((city) => ({
      ...
      orders: city.orders,
      customers: city.customerSet.size,
      bookedGMV: city.bookedGMV,
      activeGMV: city.activeGMV,
      areaManagers: city.managerSet.size,
      sales: city.salesSet.size,
    }))
    .sort((a, b) => b.bookedGMV - a.bookedGMV);
}
...
export default function GTPerformanceClient({ initialData }: { initialData?: DashboardData | null }) {
  ...
  const liveRegionalManagers = (data?.managers.regional?.length ? data.managers.regional : regionalManagers) as ManagerLike[];
  const liveAreaManagers = (data?.managers.area?.length ? data.managers.area : areaManagers) as ManagerLike[];
  const gtTransactions = useMemo(
    () => (data?.geoTransactions ?? []).filter((order) => order.channelKey === 'gt'),
    [data],
  );
  const gtSales = useMemo(() => buildSalesPerformance(gtTransactions), [gtTransactions]);
  const gtCities = useMemo(() => buildCityPerformance(gtTransactions), [gtTransactions]);
  ...
  const totalGTGMV = gtChannel?.bookedGMV ?? liveRegionalManagers.reduce((sum, manager) => sum + manager.bookedGMV, 0);
  const activeGTGMV = gtChannel?.activeGMV ?? liveRegionalManagers.reduce((sum, manager) => sum + manager.activeGMV, 0);
  const totalGTOrders = gtChannel?.orders ?? liveRegionalManagers.reduce((sum, manager) => sum + manager.orders, 0);
  const activeGTOrders = gtChannel?.activeOrders ?? liveRegionalManagers.reduce((sum, manager) => sum + manager.activeOrders, 0);
  const totalGTCustomers = gtRetentionChannel.uniqueCustomers || liveRegionalManagers.reduce((sum, manager) => sum + manager.customers, 0);
    ...
    <div className="relative animate-fade-in-up space-y-4 sm:space-y-5 lg:space-y-6">
      ...
      <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
        ...
        >
          {regionalDonutRows.length ? (
            <div className="grid min-w-0 gap-4 2xl:grid-cols-[minmax(320px,0.98fr)_minmax(280px,0.72fr)] 2xl:items-center">
              <div className="relative min-w-0 overflow-hidden rounded-[8px] border border-border/80 bg-background/30">
                ...
                <div className="pointer-events-none absolute left-1/2 top-1/2 z-10 flex w-[46%] -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center text-center">
                  <p className="text-lg font-black leading-none text-foreground drop-shadow-[0_0_18px_rgba(6,182,212,0.26)] sm:text-2xl lg:text-4xl">
                    {abbreviateIDR(donutCenterValue)}
                  </p>
                  <p className="mt-3 text-[11px] font-semibold leading-4 text-muted-foreground sm:text-xs lg:text-sm">
                    {donutCenterLabel}
                  </p>
                </div>
                <EChart
                  ...
                  style={{ height: 'clamp(340px, 48vw, 560px)', minHeight: 340 }}
                />
              </div>
              ...
              <div className="min-w-0">
                <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-2xl font-semibold leading-tight text-foreground sm:text-3xl">
                ...
                <div className="max-h-[380px] space-y-2 overflow-y-auto pr-1 2xl:max-h-[500px]">
                  {regionalDonutRows.map((row, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'group w-full min-w-0 rounded-[8px] border p-3 text-left transition',
                          isSelected
                        ...
                        <div className="flex min-w-0 items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex min-w-0 items-start justify-between gap-3">
                              ...
                              <span className="shrink-0 text-xl font-black leading-none text-foreground sm:text-2xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <div className="relative overflow-hidden rounded-[8px] border border-border bg-card p-4 shadow-sm shadow-black/10">
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-2 truncate text-2xl font-semibold text-foreground">{topRegional?.name ?? 'N/A'}</p>
                <p className="mt-2 text-xs leading-5 text-muted-foreground">
                  {topRegional ? `${abbreviateIDR(topRegional.bookedGMV)} GMV, ${formatPercent(regionalShare)} of GT` : 'No regional data'}
                </p>
              </div>
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] border border-cyan-400/30 bg-cyan-400/8 text-cyan-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-[8px] border border-border bg-card p-4 shadow-sm shadow-black/10">
              ...
              <p className="mt-2 text-xl font-semibold text-foreground">{abbreviateIDR(Math.max(0, regionalGap))}</p>
            ...
            <div className="rounded-[8px] border border-border bg-card p-4 shadow-sm shadow-black/10">
              ...
              <p className="mt-2 text-xl font-semibold text-foreground">{formatPercent(selectedRegionalActiveRate)}</p>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <GtMetricCard
          ...
          value={formatNumber(gtSales.length)}
          meta={`${formatNumber(gtCities.length)} active cities mapped`}
