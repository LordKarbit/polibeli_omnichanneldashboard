...
export default function ExecutiveOverviewPage() {
  ...
  const channelContributions = useMemo(
    () => groupExecutiveChannels((data?.channels ?? []) as ChannelLike[]),
    [data],
  );
  const dailyGMV = (data?.dailyGMV ?? []) as DailyLike[];
  ...
  const alerts: AlertLike[] = channelContributions
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <ExecutiveMetricCard
            ...
            value={data?.hasData ? `${formatPercent(data.dataQuality.validPercent)}` : 'Waiting'}
            meta={data ? `${formatNumber(data.dataQuality.validRows)} valid rows, ${formatNumber(data.dataQuality.warningIssues)} warnings` : 'Loading live reporting data'}
