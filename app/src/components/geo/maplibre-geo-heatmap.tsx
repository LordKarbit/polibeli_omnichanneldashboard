...
export interface MapLibreGeoHeatmapHandle {
...
const BUILDING_3D_SOURCE_ID = "geo-sales-openfreemap-buildings";
const BUILDING_3D_LAYER_ID = "geo-sales-3d-buildings";
const BUILDING_3D_TILEJSON_URL = "https://tiles.openfreemap.org/planet";
const BUILDING_3D_MIN_ZOOM = 15;
const MAP_MAX_ZOOM = 18.5;
const ORBIT_DRAG_SENSITIVITY = { bearing: 0.34, pitch: 0.24 };
...
function buildingNameFromProperties(properties: Record<string, unknown>) {
  return (
    asText(properties.name) ??
    asText(properties["name:id"]) ??
    asText(properties.name_en) ??
    asText(properties.building) ??
    null
  );
}

function buildingAddressFromProperties(properties: Record<string, unknown>) {
  const parts = [
    asText(properties["addr:housenumber"]),
    asText(properties["addr:street"]),
    asText(properties["addr:suburb"]),
    asText(properties["addr:city"]),
  ].filter((value): value is string => Boolean(value));

  return parts.length ? parts.join(", ") : null;
}

function getBuilding3DPaint(isLightMapSurface: boolean): Record<string, unknown> {
  return {
    "fill-extrusion-color": [
      ...
      0,
      isLightMapSurface ? "#d8e1ec" : "#243044",
      80,
      isLightMapSurface ? "#afbdcc" : "#3b4b63",
      220,
      isLightMapSurface ? "#7f8fa3" : "#5b6e8a",
    ],
    ...
    "fill-extrusion-opacity": isLightMapSurface ? 0.72 : 0.62,
    "fill-extrusion-vertical-gradient": true,
  };
}

function ensure3DBuildingsLayer(map: MapLibreMap, isLightMapSurface: boolean) {
  try {
    if (!map.getSource(BUILDING_3D_SOURCE_ID)) {
      map.addSource(BUILDING_3D_SOURCE_ID, {
        type: "vector",
        url: BUILDING_3D_TILEJSON_URL,
    ...
    if (!map.getLayer(BUILDING_3D_LAYER_ID)) {
      map.addLayer({
        ...
        paint: getBuilding3DPaint(isLightMapSurface),
      } as Parameters<MapLibreMap["addLayer"]>[0]);
    }
  } catch (error) {
    console.warn("3D building layer is unavailable", error);
  }
}
...
function update3DBuildingsLayer(
  map: MapLibreMap,
  enabled: boolean,
  isLightMapSurface: boolean,
) {
  if (!map.getLayer(BUILDING_3D_LAYER_ID)) {
    ensure3DBuildingsLayer(map, isLightMapSurface);
  }

  if (!map.getLayer(BUILDING_3D_LAYER_ID)) return;

  map.setLayoutProperty(BUILDING_3D_LAYER_ID, "visibility", enabled ? "visible" : "none");
  const paint = getBuilding3DPaint(isLightMapSurface);
  Object.entries(paint).forEach(([key, value]) => {
    map.setPaintProperty(BUILDING_3D_LAYER_ID, key, value);
  });
}

function installOrbitDragInteraction(map: MapLibreMap) {
  const canvas = map.getCanvas();
  const getOrbitModeActive = () => canvas.dataset.orbitMode === "true";
  const getOrbitKeyActive = () => canvas.dataset.orbitKeyActive === "true";
  let orbitState: {
    active: boolean;
    startX: number;
    startY: number;
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    const dx = event.clientX - orbitState.startX;
    const dy = event.clientY - orbitState.startY;

    map.jumpTo({
      bearing: orbitState.startBearing + dx * ORBIT_DRAG_SENSITIVITY.bearing,
      pitch: clamp(orbitState.startPitch - dy * ORBIT_DRAG_SENSITIVITY.pitch, 0, 62),
    });
  };
  ...
  const handleMouseDown = (event: MouseEvent) => {
    const shouldOrbit = getOrbitModeActive() && (event.button === 1 || (event.button === 0 && getOrbitKeyActive()));
...
function getTimestampFilenamePart() {
...
function triggerPNGDownload(canvas: HTMLCanvasElement, filename: string) {
  ...
  try {
    ...
    window.alert("Map export failed. Please wait until map tiles finish loading, then try again.");
  }
}
...
function drawExportPanel(
  ...
  const x = 24 * scale;
  const y = 24 * scale;
  const width = Math.min(360 * scale, ctx.canvas.width - 48 * scale);
  const height = 104 * scale;

  ctx.save();
  roundedRectPath(ctx, x, y, width, height, 12 * scale);
  ctx.fillStyle = isClearMap ? "rgba(248, 250, 252, 0.9)" : "rgba(2, 6, 23, 0.88)";
  ctx.fill();
  ctx.strokeStyle = isClearMap ? "rgba(15, 23, 42, 0.18)" : "rgba(148, 163, 184, 0.28)";
  ctx.lineWidth = scale;
  ctx.stroke();

  ctx.fillStyle = isClearMap ? "rgba(8, 47, 73, 0.78)" : "rgba(165, 243, 252, 0.88)";
  ctx.font = `${10 * scale}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
  ctx.fillText(title.toUpperCase(), x + 16 * scale, y + 27 * scale);

  ctx.fillStyle = isClearMap ? "#020617" : "#ffffff";
  ctx.font = `700 ${22 * scale}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
  ctx.fillText(abbreviateIDR(mappedGMV), x + 16 * scale, y + 58 * scale);

  ctx.fillStyle = isClearMap ? "rgba(15, 23, 42, 0.68)" : "rgba(203, 213, 225, 0.78)";
  ctx.font = `${12 * scale}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
  ctx.fillText(`${boundaryCount.toLocaleString("id-ID")} visible boundaries by ${metricLabel}`, x + 16 * scale, y + 84 * scale);
  ctx.restore();
}

function drawExportLegend(ctx: CanvasRenderingContext2D, scale: number, metricLabel: string, isClearMap: boolean) {
  const width = 288 * scale;
  const height = 46 * scale;
  const x = 24 * scale;
  const y = ctx.canvas.height - height - 24 * scale;

  ctx.save();
  roundedRectPath(ctx, x, y, width, height, 12 * scale);
  ctx.fillStyle = isClearMap ? "rgba(248, 250, 252, 0.9)" : "rgba(2, 6, 23, 0.86)";
  ctx.fill();
  ctx.strokeStyle = isClearMap ? "rgba(15, 23, 42, 0.18)" : "rgba(148, 163, 184, 0.26)";
  ctx.lineWidth = scale;
  ctx.stroke();

  ctx.fillStyle = "#22d3ee";
  ctx.beginPath();
  ctx.arc(x + 22 * scale, y + 23 * scale, 5 * scale, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = isClearMap ? "rgba(15, 23, 42, 0.72)" : "rgba(226, 232, 240, 0.9)";
  ctx.font = `${12 * scale}px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
  ctx.fillText(`Low ${metricLabel}`, x + 36 * scale, y + 27 * scale);

  const gradientX = x + 120 * scale;
  const gradientWidth = 82 * scale;
  ...
  roundedRectPath(ctx, gradientX, y + 17 * scale, gradientWidth, 10 * scale, 5 * scale);
  ctx.fillStyle = gradient;
  ctx.fill();

  ctx.fillStyle = isClearMap ? "rgba(15, 23, 42, 0.72)" : "rgba(226, 232, 240, 0.9)";
  ctx.fillText(`High ${metricLabel}`, x + 216 * scale, y + 27 * scale);
  ctx.restore();
}

function waitForMapPaint(map: MapLibreMap) {
...
function getLineColor(gmv: number, maxGMV: number, isLightSurface: boolean) {
  const ratio = gmvRatio(gmv, maxGMV);

  if (isLightSurface) {
    return [100, 116, 139, 132];
  }
...
export const MapLibreGeoHeatmap = forwardRef<MapLibreGeoHeatmapHandle, MapLibreGeoHeatmapProps>(
function MapLibreGeoHeatmap({ locations, transactions = [], className }, ref) {
  ...
  const buildingTooltipEnabledRef = useRef(false);
  const buildingLookupCacheRef = useRef<Map<string, Omit<BuildingHoverState, "x" | "y" | "height" | "isLoading">>>(new Map());
  const buildingHoverTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const buildingHoverAbortRef = useRef<AbortController | null>(null);
  const buildingHoverRequestIdRef = useRef(0);
  const orbitKeyActiveRef = useRef(false);
  ...
  const [buildingHover, setBuildingHover] = useState<BuildingHoverState | null>(null);
  ...
  const effectiveClearMap = isClearMap;
  const isLightMapSurface = effectiveClearMap || dashboardTheme === "light";
  const canShow3DBuildings = currentZoom >= BUILDING_3D_MIN_ZOOM - 0.15;
  const cancelBuildingHoverLookup = useCallback(() => {
    if (buildingHoverTimerRef.current) {
      clearTimeout(buildingHoverTimerRef.current);
      buildingHoverTimerRef.current = null;
    }

    buildingHoverAbortRef.current?.abort();
    buildingHoverAbortRef.current = null;
    buildingHoverRequestIdRef.current += 1;
  }, []);
  const clearBuildingHover = useCallback(() => {
    cancelBuildingHoverLookup();
  ...
  const downloadPNG = useCallback(async () => {
    ...
    ctx.fillStyle = isLightMapSurface ? "#e5edf4" : "#020617";
    ...
    try {
      canvases.forEach((canvas) => {
        ...
        const destinationX = (canvasRect.left - rect.left) * scale;
        const destinationY = (canvasRect.top - rect.top) * scale;
        const destinationWidth = canvasRect.width * scale;
        const destinationHeight = canvasRect.height * scale;

        ctx.drawImage(canvas, destinationX, destinationY, destinationWidth, destinationHeight);
      });
      ...
      drawExportPanel(
        ...
        boundaryMetricTitle,
        mappedGMV,
        activeBoundaries.features.length,
        metricLabel,
        isLightMapSurface,
      );
      drawExportLegend(ctx, scale, metricLabel, isLightMapSurface);

      triggerPNGDownload(exportCanvas, `indonesia-gmv-boundary-map-${getTimestampFilenamePart()}.png`);
    } catch (error) {
      console.error("Map export failed", error);
      window.alert("Map export failed. Please wait until map tiles finish loading, then try again.");
    }
  }, [activeBoundaries.features.length, boundaryMetricTitle, isLightMapSurface, mappedGMV, metricLabel]);

  useImperativeHandle(ref, () => ({ downloadPNG }), [downloadPNG]);

  useEffect(() => {
    const syncTheme = () => setDashboardTheme(currentDashboardTheme());
    syncTheme();
  ...
  useEffect(() => {
    const enabled = show3DBuildings && canShow3DBuildings;
    buildingTooltipEnabledRef.current = enabled;

    if (!enabled) {
      cancelBuildingHoverLookup();
    }
  }, [canShow3DBuildings, cancelBuildingHoverLookup, show3DBuildings]);

  useEffect(() => {
    let cancelled = false;
    let cleanupOrbitDrag: (() => void) | null = null;
    ...
    async function initializeMap() {
      ...
      try {
        ...
        cleanupOrbitDrag = installOrbitDragInteraction(map);
  ...
  useEffect(() => {
    ...
    map.setPaintProperty("geo-background", "background-color", isLightMapSurface ? "#e5edf4" : "#020617");
    map.setPaintProperty("osm-basemap", "raster-opacity", !effectiveClearMap && dashboardTheme === "dark" ? 1 : 0);
    map.setPaintProperty("carto-light-basemap", "raster-opacity", !effectiveClearMap && dashboardTheme === "light" ? 1 : 0);
    map.setPaintProperty("clear-basemap", "raster-opacity", effectiveClearMap ? 1 : 0);
  }, [dashboardTheme, effectiveClearMap, isLightMapSurface, isReady]);

  useEffect(() => {
    const map = mapRef.current;
    if (!map || !isReady) return;

    update3DBuildingsLayer(map, show3DBuildings && canShow3DBuildings, isLightMapSurface);
  }, [canShow3DBuildings, isLightMapSurface, isReady, show3DBuildings]);
  ...
  useEffect(() => {
    ...
    const canvas = map.getCanvas();
    const orbitModeEnabled = show3DBuildings && canShow3DBuildings;
    canvas.dataset.orbitMode = orbitModeEnabled ? "true" : "false";

    const setOrbitKeyActive = (active: boolean) => {
      orbitKeyActiveRef.current = active && orbitModeEnabled;
      canvas.dataset.orbitKeyActive = orbitKeyActiveRef.current ? "true" : "false";
      if (orbitKeyActiveRef.current) {
    ...
    setOrbitKeyActive(false);

    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key.toLowerCase() !== "o" || event.repeat) return;
      const target = event.target as HTMLElement | null;
      if (target?.closest("input, textarea, select, [contenteditable='true']")) return;
      setOrbitKeyActive(true);
    };
    ...
    const handleWindowBlur = () => setOrbitKeyActive(false);
    ...
    return () => {
      setOrbitKeyActive(false);
      canvas.dataset.orbitMode = "false";
      window.removeEventListener("keydown", handleKeyDown);
      window.removeEventListener("keyup", handleKeyUp);
      window.removeEventListener("blur", handleWindowBlur);
    };
  }, [canShow3DBuildings, isReady, show3DBuildings]);
  ...
  useEffect(() => {
    ...
    const layer = new GeoJsonLayer({
      ...
      pickable: true,
      autoHighlight: true,
      highlightColor: isLightMapSurface ? [15, 23, 42, 58] : [255, 255, 255, 72],
      stroked: showBoundaries,
      filled: showGMVBlocks,
      lineWidthUnits: "pixels",
      lineWidthMinPixels: isLightMapSurface ? 0.9 : 1,
      lineWidthMaxPixels: isLightMapSurface ? 1.25 : 4,
      ...
      getLineColor: (feature: Feature<Geometry, CityBoundaryProperties>) =>
        getLineColor(feature.properties?.totalGMV ?? 0, maxGMV, isLightMapSurface),
      getLineWidth: (feature: Feature<Geometry, CityBoundaryProperties>) =>
        isLightMapSurface ? 1 : 1 + gmvRatio(feature.properties?.totalGMV ?? 0, maxGMV) * 2.6,
      updateTriggers: {
        getFillColor: [maxGMV, gmvOpacity],
        getLineColor: [maxGMV, isLightMapSurface],
        getLineWidth: [maxGMV, isLightMapSurface],
      },
  ...
  }, [activeBoundaries, deckReadyVersion, gmvOpacity, isLightMapSurface, maxGMV, showBoundaries, showGMVBlocks]);
  ...
  function toggle3DBuildings() {
    ...
    if (next) {
      map.easeTo({
        zoom: Math.max(map.getZoom(), BUILDING_3D_MIN_ZOOM + 0.35),
        pitch: 58,
  ...
  function showBuildingTooltipAtPoint(point: { x: number; y: number }, lngLat: { lat: number; lng: number }) {
    const map = mapRef.current;

    if (!map || !buildingTooltipEnabledRef.current) return false;

    let features: Array<{ id?: string | number; properties?: Record<string, unknown> }> = [];
    try {
      if (map.getLayer(BUILDING_3D_LAYER_ID)) {
        features = map.queryRenderedFeatures([point.x, point.y], { layers: [BUILDING_3D_LAYER_ID] }) as typeof features;
      }
    } catch {
      features = [];
    }
    ...
    const tooltipPoint = getTooltipPosition(mapContainerRef.current, point.x, point.y);
    const lat = lngLat.lat;
    const lon = lngLat.lng;
    const lookupKey = building.id ? `feature:${building.id}` : `${lat.toFixed(5)},${lon.toFixed(5)}`;
    const tileName = buildingNameFromProperties(building.properties);
    const tileAddress = buildingAddressFromProperties(building.properties);
    const height = asNumber(building.properties.render_height);
    const cached = buildingLookupCacheRef.current.get(lookupKey);
    ...
    setBuildingHover({
      x: tooltipPoint.x,
      y: tooltipPoint.y,
      title: tileName ?? "Building footprint",
      address: tileAddress ?? "Resolving nearest address...",
      category: null,
      height,
      isLoading: !tileAddress,
    });

    if (tileName && tileAddress) {
      buildingLookupCacheRef.current.set(lookupKey, {
        title: tileName,
        address: tileAddress,
        category: null,
      });
    ...
    const requestId = buildingHoverRequestIdRef.current + 1;
    buildingHoverRequestIdRef.current = requestId;

    buildingHoverTimerRef.current = setTimeout(async () => {
      buildingHoverAbortRef.current?.abort();
      const controller = new AbortController();
      buildingHoverAbortRef.current = controller;

      try {
        const response = await fetch(`/api/geo/reverse?lat=${lat.toFixed(6)}&lon=${lon.toFixed(6)}`, {
          cache: "force-cache",
        ...
        const payload = (await response.json()) as {
          ok?: boolean;
          data?: { title?: string; address?: string; category?: string | null };
        };
        const data = payload.data;

        if (buildingHoverRequestIdRef.current !== requestId || !data) {
          return;
        }

        const resolved = {
          title: tileName ?? data.title ?? "Building footprint",
          address: tileAddress ?? data.address ?? "Address unavailable",
          category: data.category ?? null,
        };

        buildingLookupCacheRef.current.set(lookupKey, resolved);
        setBuildingHover((current) =>
          current
            ? {
                ...current,
                ...resolved,
        ...
        if (buildingHoverRequestIdRef.current !== requestId) return;

        setBuildingHover((current) =>
          current
            ? {
                ...current,
                title: tileName ?? current.title,
                address: tileAddress ?? "Address unavailable",
                isLoading: false,
              }
            : null,
        );
  ...
  function handleMapContextMenu(event: ReactMouseEvent<HTMLDivElement>) {
    if (!buildingTooltipEnabledRef.current) return;
    ...
    if (!showBuildingTooltipAtPoint(point, { lat: lngLat.lat, lng: lngLat.lng })) {
      clearBuildingHover();
    }
  }
  ...
  return (
      <div
        ...
        className={cn(
        "geo-sales-map-surface relative h-[520px] min-h-[520px] overflow-hidden rounded-[8px] border border-slate-800 bg-slate-950 sm:h-[580px] sm:min-h-[580px] lg:h-[640px] lg:min-h-[640px]",
        className,
      )}
    >
      <div
      ...
      {buildingHover && show3DBuildings && canShow3DBuildings ? (
        <div
          className="pointer-events-none absolute z-50 w-[min(300px,calc(100vw-2rem))] rounded-[8px] border border-indigo-300/35 bg-slate-950/94 p-3 text-xs text-slate-100 shadow-2xl shadow-slate-950/50 backdrop-blur"
          style={{
            left: buildingHover.x,
            top: buildingHover.y,
          }}
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-indigo-200/90">
              <Building2 className="h-3.5 w-3.5" />
              3D Building
            </div>
            {buildingHover.isLoading ? (
              <span className="rounded-full border border-indigo-300/25 bg-indigo-400/10 px-2 py-0.5 text-[10px] text-indigo-100">
                resolving
              </span>
            ) : null}
          </div>
          <div className="mt-2 text-sm font-semibold leading-5 text-white">{buildingHover.title}</div>
          <div className="mt-1 text-[11px] leading-4 text-slate-400">{buildingHover.address}</div>
          <div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-800 pt-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-slate-500">Height</p>
              <p className="mt-1 font-semibold text-slate-100">
                {buildingHover.height ? `${Math.round(buildingHover.height)} m` : "Unknown"}
              </p>
            </div>
            <div>
                ...
                {canShow3DBuildings ? (
                  <button
                    ...
                    className={cn(
                      "flex h-8 items-center justify-between gap-3 rounded-[8px] border px-2.5 text-left text-[11px] font-medium transition",
                      show3DBuildings
                        ? "border-indigo-300/70 bg-indigo-500/14 text-indigo-100"
                        : "border-slate-700/70 bg-slate-900/80 text-slate-300 hover:text-slate-100",
                    )}
                  >
                    <span className="flex items-center gap-2">
                    ...
                    <span
                      className={cn(
                        "relative h-4 w-7 rounded-full transition",
                        show3DBuildings ? "bg-indigo-400/80" : "bg-slate-700",
                      )}
                    >
                      <span
                        className={cn(
                          "absolute top-0.5 h-3 w-3 rounded-full bg-white transition",
                          show3DBuildings ? "left-3.5" : "left-0.5",
                        )}
                      />
                    </span>
                  </button>
