...
const navItems = [
  ...
  { href: '/income', label: 'Income & Settlement', icon: WalletCards },
...
export function Sidebar() {
  ...
  const { permissions, roleLabel } = useDashboardAccess();
  const [collapsed, setCollapsed] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);
  const previousPathname = useRef(pathname);
  const accessibleNavItems = navItems.filter((item) => canAccessPath(item.href, permissions));
      ...
      <div className="flex h-16 items-center gap-3 border-b border-border px-4">
        {(!collapsed || mobileOpen) && (
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-[10px] text-muted-foreground">{roleLabel} Workspace</span>
          </div>
        )}
        {/* Mobile close button */}
        <button
